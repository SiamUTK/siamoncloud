...
const CHATBOT_AVATAR_SRC = "/images/ai-mascot/bindee-head-1.png";
...
function Chatbot() {
  ...
  const greeting = t("chatbot_greeting");
    ...
    try {
      if (hasSupabaseConfig) {
        ...
        if (response.ok) {
          ...
          setMessages((prev) => [
            ...prev,
            { sender: "ai", text: data?.reply || t("chatbot_response") },
          ]);
        } else {
          setMessages((prev) => [
            ...prev,
            { sender: "ai", text: t("chatbot_response") },
          ]);
        }
      } else {
        setTimeout(() => {
  ...
  return (
    <div
      ...
      aria-label={t("chatbot_aria_assistant")}
    >
      {isOpen && (
        <div
          ...
          role="dialog"
          aria-label={t("chatbot_aria_dialog")}
        >
          <div className="flex items-center justify-between bg-gradient-to-r from-[#0A2540] to-[#06B6D4] p-4 text-white shadow-md">
            ...
            <button
              ...
              aria-label={t("chatbot_close_chat")}
            >
